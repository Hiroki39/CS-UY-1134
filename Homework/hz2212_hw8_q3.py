...
def restore_bst(prefix_lst):
    def restore_bst_helper(prefix_lst, start_pos, val_to_insert):  # returns tuples containing the connect position and current node
        new_item = BinarySearchTreeMap.Item(prefix_lst[start_pos])
        curr_node = BinarySearchTreeMap.Node(new_item)  # create subtrees leaning to left
        if start_pos == 0:
        ...
        prev_connect_pos, curr_root = restore_bst_helper(prefix_lst, start_pos - 1, curr_node.item.key)
        prev_connect_pos.right = curr_node
        curr_node.parent = prev_connect_pos  # connect current subtree to the returned subtree
        ...
        if val_to_insert is not None and val_to_insert > curr_root.item.key > connect_pos.item.key:
            return curr_root, curr_root  # process the edge case in which current subtree should connect to the root
